...
```{r noise, echo = FALSE,}
happy1 <- c("4","4","4","4","4","4","4","4","4","4","*M* = 4")
sad1 <- c("3","3","3","3","3","3","3","3","3","3","*M* = 3")
table1 <- data.frame(happy1,sad1)

happy2 <- c("3","6","2","4","5","2","3","1","6","8","*M* = 4")
sad2 <- c("1","3","4","0","5","7","2","5","1","2","*M* = 3")
table2 <- data.frame(happy2,sad2)
...
knitr::kable(
  ...
  col.name=c("Happy","Sad"), caption = 'Left: Hypothetical noiseless data; Right: realistic noisy data')
```
...
```{r confound, echo = FALSE, out.width='70%', fig.align = 'center', fig.cap='Hypothetical results from a study on the effect of mood on memory. Because IQ also differs across conditions, it is a confounding variable.', fig.alt = 'Bar chart showing memory performance (y-axis, 0 to 15) by mood condition (x-axis). The positive mood condition bar (blue) reaches approximately 14, and the negative mood condition bar (red) reaches approximately 8.5.'}
memory <- c(14.0,8.5)

barplot(memory, names.arg = c("Positive (Higher IQ)","Negative (Lower IQ)"), col = c("#4E67C8","#F14124"), xlab = "Mood Condition", axis.lty=1, ylab = "Memory Performance", ylim = c(0,16))
```
...
cond <- c("**A**","**C**","**B**","B","C","A","**C**","**B**","**A**")
block <- data.frame(participant,cond)

knitr::kable(block,
  col.name=c("Participant","Condition"), caption = 'Block randomization sequence for assigning nine participants to three conditions.')
```
...
```{r placebo, echo = FALSE, out.width='80%', fig.align = 'center', fig.cap='Hypothetical results from a study including treatment, no-treatment, and placebo conditions.', fig.alt = 'Bar chart showing improvement scores (y-axis, 0 to 8) across three conditions. The treatment group (light blue) shows the highest improvement at approximately 7, followed by the placebo group (red) at approximately 5, and the no treatment group (blue) at approximately 3.'}
improvement <- c(7,3,5)

barplot(improvement, names.arg = c("Treatment","No Treatment","Placebo"), col = c("#1BADE3","#4E67C8","#F14124"), xlab = "Condition", axis.lty=1, ylab = "Improvement", ylim = c(0,8))
```
...
```{r surgery, echo = FALSE, out.width='50%', fig.align = 'center', fig.cap='Research has shown that patients with osteoarthritis of the knee who receive a “sham surgery” experience reductions in pain and improvement in knee function similar to those of patients who receive a real surgery. *Photo by Piron Guillaume on Unsplash.*', fig.alt = 'Photograph of a rack of surgical instruments in focus in the foreground, with blurred figures in scrubs gathered around a surgical table visible in the background.'}
knitr::include_graphics("images/experiments/surgery.jpeg")
```
:::
