...
```{r writing, echo = FALSE, out.width='40%', fig.align = 'center', fig.cap='Scientific research has shown that engaging in expressive writing causes improvements in health. Several theories have been proposed to explain this phenomenon. *Photo by lilartsy on Unsplash.*', fig.alt = 'Close-up photograph of a hand writing longhand in a notebook.'}
knitr::include_graphics("images/theory/writing.jpeg")
```
...
```{r memory, echo = FALSE, out.width='75%', fig.align = 'center', fig.cap='Representation of the multistore model of human memory. In the multistore model of human memory, information from the environment passes through a sensory store on its way to a short-term store, where it can be rehearsed, and then to a long-term store, where it can be stored and retrieved much later. This theory has been extremely successful at organizing old phenomena and predicting new ones.', fig.alt = 'Flowchart of a memory model. An arrow from the environment points to the sensory store, which feeds into the short-term store, which has a bidirectional arrow connecting it to the long-term store. All three stores have separate arrows pointing to the word "forgetting."'}
knitr::include_graphics("images/theory/memory.png")
```
...
```{r g, echo = FALSE, out.width='45%', fig.align = 'center', fig.cap='Representation of one theory of intelligence. In this theory of intelligence, a general mental ability (*g*) influences each of three more specific mental abilities. Theories of this type help to organize a large number of statistical relationships among tests of various mental abilities.', fig.alt = 'Diagram illustrating Spearman\'s general intelligence factor. A box labeled g at the top points downward to three boxes representing specific abilities: numerical ability, spatial ability, and verbal ability.'}
knitr::include_graphics("images/theory/g.png")
```
...
```{r hypothetico, echo = FALSE, out.width='75%', fig.align = 'center', fig.cap='Hypothetico-deductive method combined with the general model of scientific research in psychology. Together they form a model of theoretically motivated research.', fig.alt = 'A diagram combining the hypothetico-deductive method with the general model of scientific research in psychology, shown as two concentric cycles. The outer cycle moves clockwise through four stages: theory construction and revision, hypothesis derivation, hypothesis testing, and theory evaluation, which feeds back into theory construction. The inner cycle moves through five stages: research questions, empirical study, data analysis, conclusions, and research literature, which feeds back to research questions. The inner cycle also includes a loop back from conclusions to research literature. An external arrow labeled "informal observations/ practical problems" points inward to the research questions stage of the inner cycle.'}
knitr::include_graphics("images/theory/hypothetico.png")
```
