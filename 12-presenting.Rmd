...
```{r values, echo = FALSE, }
feature <- c("There are very few direct quotations of other researchers.","Criticisms are directed at other researchers’ work but not at them personally.","There are many references and reference citations.","Empirical research reports are organized with specific sections in a fixed order.","Researchers tend to “hedge” their conclusions, e.g., “The results suggest that…”")
value <- c("The phenomena and theories of psychology are objective and do not depend on the specific words a particular researcher used to describe them.","The focus of scientific research is on drawing general conclusions about the world, not on the personalities of particular researchers.","Scientific research is a large-scale collaboration among many researchers.","There is an ideal approach to conducting empirical research in psychology (even if this ideal is not always achieved in actual research).","Scientific knowledge is tentative and always subject to revision based on new empirical results.")
values <- data.frame(feature,value)

knitr::kable(values,
  col.name=c("APA style feature","Scientific value or assumption"))
```
:::
...
```{r bias, echo = FALSE, }
instead <- c("man, men","firemen","homosexuals, gays, bisexuals","minority","neurotics","special children")
use <- c("men and women, people","firefighters","lesbians, gay men, bisexual men, bisexual people","specific group label (e.g., African American)","people scoring high in neuroticism","children with learning disabilities")
bias <- data.frame(instead,use)

knitr::kable(bias,
  col.name=c("Instead of...","Use..."), caption = 'Examples of avoiding biased language.')
```
...
```{r errors, echo = FALSE, }
error <- c("1. Use of numbers","2. Hyphenation","3. Use of et al.","4. Headings","5. Use of since","6. Tables and figures","7. Use of commas","8. Use of abbreviations","9. Spacing","10. Use of & in references")
example <- c("Failing to use numerals for 10 and above","Failing to hyphenate compound adjectives that precede a noun (e.g., “role playing technique” should be “role-playing technique”)","Failing to use it in a reference","Not capitalizing headings correctly","Using since to mean because","Not formatting them in APA style; repeating information that is already given in the text","Failing to use a comma before and or or in a series of three or more elements","Failing to spell out a term completely before introducing an abbreviation for it","Not consistently double-spacing between lines","Using & in the text or *and* in parentheses")
errors <- data.frame(error,example)

knitr::kable(errors,
  col.name=c("Error type","Example"), caption = 'Top 10 APA-Style Errors.')
```
...
```{r sample1, echo = FALSE, out.width='65%', fig.align = 'center', fig.cap='Title Page and Abstract. This paper does not include the author note on the title page. The abstract appears on its own page.', fig.alt = 'Screenshot of an APA-style research paper showing the title page and abstract. The title page includes the paper title, author name, and institutional affiliation but no author note. The abstract appears on a separate page.'}
knitr::include_graphics("images/ch11/sample1.png")
```

```{r sample2, echo = FALSE, out.width='65%', fig.align = 'center', fig.cap='Introduction and Method. Note that the introduction is headed with the full title, and the method section begins immediately after the introduction ends.', fig.alt = 'Screenshot of an APA-style research paper showing the introduction and method sections. The introduction is headed with the full title of the paper rather than the word Introduction, and the method section begins immediately after the introduction ends on the same page.'}
knitr::include_graphics("images/ch11/sample2.png")
```

```{r sample3, echo = FALSE, out.width='65%', fig.align = 'center', fig.cap='Results and Discussion. The discussion begins immediately after the results section ends.', fig.alt = 'Screenshot of an APA-style research paper showing the results and discussion sections. The discussion section begins immediately after the results section ends on the same page.'}
knitr::include_graphics("images/ch11/sample3.png")
```

```{r sample4, echo = FALSE, out.width='65%', fig.align = 'center', fig.cap='References and Figure. If there were appendixes or tables, they would come before the figure.', fig.alt = 'Screenshot of an APA-style research paper showing the references page followed by a figure. The references are formatted in APA style, and the figure appears on its own page after the references.'}
knitr::include_graphics("images/ch11/sample4.png")
```
...
```{r poster, echo = FALSE, out.width='60%', fig.align = 'center', fig.cap='New research is often presented in the form of posters at professional conferences. *Photo by Peter Macdonald on Flickr.*', fig.alt = 'Photograph of a student presenting a research poster mounted on a wall at an academic conference poster session, gesturing toward the poster while a professor listens attentively. Both wear name tags and the room is crowded with other attendees in the background.'}
knitr::include_graphics("images/ch11/poster.jpeg")
```
